...
from src.helpers import mannwhitney, save_altair, signature_score, standardize
...
clusters = pd.read_csv(os.path.join(config["full"], "clusters.csv"), index_col="cell")
...
for node_id, cell_line in main_branch_dict.items():
    tfidf.obs.loc[
        (tfidf.obs_names.isin(node_dict[node_id])) & (tfidf.obs["sample"] == cell_line),
        "main_branch",
    ] = cell_line
...
save(cell_count_plot, "total_cells_plot", config["plots"])
...
save_altair(id2_plot, "id2_boxplot_dropzero_median", config["plots"])
# Calculate stats and write to file
id2_stats = mannwhitney(id2_df, "id2")
id2_stats.to_csv(os.path.join(config["plots"], "id2_stats_dropzero.csv"), index=False)
...
diapause_stats.to_csv(os.path.join(config["plots"], "diapause_stats.csv"), index=False)
...
reads = pd.DataFrame(znorm.todense(), index=adata.obs_names, columns=adata.var_names)
